...
- **tmux 1.8 以上。** 外掛只用長期穩定的功能：`@` 前綴使用者選項（tmux 1.7）、
  `show-options -q`（tmux 1.8）、狀態列的非同步 `#(shell-command)` 展開
  （tmux 1.8），已對照官方
  [tmux CHANGES](https://github.com/tmux/tmux/blob/master/CHANGES) 查證。
